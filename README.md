...
| _GerarPendentes_ | `pendentes/` | Em importação | Confronta dados e gera uma planilha de notas de mercadoria pendente de entrada. O núcleo comum das duas rotinas e a documentação do porte já estão no repositório; o motor de mercadorias é a entrega seguinte. |
| _GerarServPend_ | `pendentes/` | Em importação | Confronta dados e gera uma planilha de notas de serviço pendente de entrada. Compartilha o núcleo com a de cima — duas ferramentas na tela, um projeto no disco. |
...
em que for precisar. As duas de notas pendentes seguem apagadas enquanto os
motores não rodam — botão que não roda é pior do que botão apagado —, mas o
texto delas na tela já diz em que pé o porte está.
